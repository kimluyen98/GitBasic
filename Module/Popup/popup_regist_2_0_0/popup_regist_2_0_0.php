<?php
    $path = get_template_directory_uri();
    $path = $path.'/Module/Popup/popup_regist_2_0_0';
    if($check == 0){
        include(locate_template("Module/Popup/popup_regist_2_0_0/style.php"));
        add_action('wp_footer', 'popup_regist_2_0_0');
        function popup_regist_2_0_0(){ 
            echo '                    
              <script type="text/javascript">
                $(document).ready(function () {
                  $(".bvh_dkdl").click(function () {
                  $(".popup_regist_2_0_0 #popup_regist_2_0_0_overlayClickform").fadeIn(5);
                  $(".popup_regist_2_0_0 .popup_regist_2_0_0_overlay").fadeIn(500);
                  $("html, body").animate({ scrollTop: 0 }, "slow");
                });

                $(".popup_regist_2_0_0 #popup_regist_2_0_0_closePopup, .popup_regist_2_0_0 .popup_regist_2_0_0_overlay").click(function () {
                  $(".popup_regist_2_0_0 #popup_regist_2_0_0_overlayClickform").fadeOut(500);
                  $(".popup_regist_2_0_0 .popup_regist_2_0_0_overlay").fadeOut(500);
                  });
                });

              </script>
            ';           
          };
      }
  ?>


<div class="popup_regist_2_0_0">
  <div class="popup_regist_2_0_0_overlay" style="display:none"></div>
  <div id="popup_regist_2_0_0_overlayClickform" class="overlay_form" style="display:none">
    <div class="popup_regist_2_0_0_box">
      <article>

        <div class="popup_regist_2_0_0__header">
            <div class="pic">
              <img src="<?php echo $path ?>/images/logo.png" alt="">
            </div>
            <h2>
              Đặt lịch thăm khám
            </h2>
        </div>

        <div class="popup_regist_2_0_0__content">
          <input type="text" placeholder="Họ và tên (*)" id="iname" name="iname">
          <input type="text" placeholder="Số điện thoại (*)" id="imob" name="imob">
          <input type="text" placeholder="Địa chỉ" id="icity" name="icity">
          <input type="date" style="width:100%" placeholder="Ngày khám" id="itime" name="itime">
          <div style="display:none">
            <input type="hidden" id="gclid_field" name="gclid_field" value="">
            <input id="iemail" name="iemail" value="no@email.chuahiemmuon.vn" type="hidden" placeholder="Email">
            <input id="ititle" name="ititle" value="Đặt lịch thăm khám" type="hidden">
          </div>
        </div>

        <div class="popup_regist_2_0_0__ft">
          <div class="ft-1">
              <div class="pic">
                <img src="data:image/png;base64,
                " alt="">
              </div>
              <div class="text">
                <p>Tư vấn trực tiếp</p>
                <b>1900 633 988</b>
              </div>
          </div>
          <div class="regist">
            <button onclick="ants_send_contact(this)">Đăng ký</button>
          </div>
        </div>

        <div class="popup_regist_2_0_0__note">
          <span><b>Lưu ý:</b> Hãy để lại thông tin chính xác để chúng tôi hỗ trợ bạn trong thời gian sớm nhất.</span>
        </div>
        
        <a id="popup_regist_2_0_0_closePopup"></a>

      </article>
    </div>
  </div>
</div>






